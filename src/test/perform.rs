...
fn test_perform_read_requests() -> Result<()> {
    ...
    let get_entity_type_request = sreq![Request::GetEntityType {
    ...
    if let Request::GetEntityType { entity_type, .. } = &results.read()[0] {
        assert!(
            entity_type.is_none(),
            "Non-existent entity type should be None"
        );
    } else {
    ...
    let get_field_type_request = sreq![Request::GetFieldType {
    ...
    if let Request::GetFieldType { field_type, .. } = &results.read()[0] {
        assert!(
            field_type.is_none(),
            "Non-existent field type should be None"
        );
    } else {
    ...
    let resolve_entity_type_request = sreq![Request::ResolveEntityType {
    ...
    if let Request::ResolveEntityType { name, .. } = &results.read()[0] {
        assert!(
            name.is_none(),
            "Non-existent entity type should resolve to None"
        );
    } else {
    ...
    let resolve_field_type_request = sreq![Request::ResolveFieldType {
    ...
    if let Request::ResolveFieldType { name, .. } = &results.read()[0] {
        assert!(
            name.is_none(),
            "Non-existent field type should resolve to None"
        );
    } else {
...
fn test_perform_invalid_request() {
    ...
    let write_request = sreq![swrite!(
        EntityId(0),
        crate::sfield![FieldType(0)],
        sstr!("test")
    )];
    ...
    assert!(
        result.is_err(),
        "Write requests should fail with immutable perform"
    );
