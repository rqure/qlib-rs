...
async fn test_create_entity_hierarchy() -> Result<()> {
    ...
    let root_entities = store.find_entities(&et_root, None).await?;
...
async fn test_entity_deletion() -> Result<()> {
    ...
    // Delete the entity
    store.perform_mut(vec![sdelete!(user_ref.clone())]).await?;
    ...
    let result = store.perform_mut(request).await;
...
async fn test_entity_listing_with_pagination() -> Result<()> {
    ...
    let user_entities = store.find_entities(&et_user, None).await?;
...
async fn test_find_entities_comprehensive() -> Result<()> {
    ...
    let name_read = store.perform_mut(name_read).await?;
