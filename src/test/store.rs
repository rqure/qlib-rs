...
fn test_create_entity_hierarchy() -> Result<()> {
    ...
    // Create root entity
    let create_requests = sreq![screate!(et_folder, "Security Models".to_string())];
    let create_requests = store.perform_mut(create_requests)?;
    let root_entity_id = if let Some(Request::Create {
    ...
    let create_requests = sreq![screate!(et_folder, "Users".to_string(), root_entity_id_ref)];
    let create_requests = store.perform_mut(create_requests)?;
    let users_folder_id = if let Some(Request::Create {
        created_entity_id: Some(id),
        ..
    }) = create_requests.get(0)
    {
    ...
    let create_requests = sreq![screate!(et_folder, "Roles".to_string(), root_entity_id_ref)];
    let create_requests = store.perform_mut(create_requests)?;
    let roles_folder_id = if let Some(Request::Create {
        created_entity_id: Some(id),
        ..
    }) = create_requests.get(0)
    {
    ...
    let create_requests = sreq![screate!(et_user, "qei".to_string(), users_folder_id_ref)];
    let create_requests = store.perform_mut(create_requests)?;
    let user_id = if let Some(Request::Create {
        created_entity_id: Some(id),
        ..
    }) = create_requests.get(0)
    {
    ...
    let create_requests = sreq![screate!(et_user, "admin".to_string(), roles_folder_id_ref)];
    ...
    if let Some(Request::Read {
        value: Some(Value::EntityReference(Some(parent_id))),
        ..
    }) = reqs.get(0)
    {
    ...
    // Verify name
    let reqs = store.perform_mut(sreq![sread!(users_folder_id_ref, crate::sfield![ft_name]),])?;
...
fn test_field_operations() -> Result<()> {
    ...
    let create_requests = sreq![screate!(et_folder, "Users".to_string())];
    let create_requests = store.perform_mut(create_requests)?;
    let users_folder_id = if let Some(Request::Create {
        created_entity_id: Some(id),
        ..
    ...
    let create_requests = sreq![screate!(et_user, "testuser".to_string(), users_folder_id)];
    let create_requests = store.perform_mut(create_requests)?;
    let user_id = if let Some(Request::Create {
        created_entity_id: Some(id),
        ..
    ...
    let ft_name = store.get_field_type("Name")?;
    store.perform_mut(sreq![swrite!(
        user_ref,
        crate::sfield![ft_name],
        sstr!("Updated User")
    ),])?;

    let reads = store.perform_mut(sreq![sread!(user_ref, crate::sfield![ft_name]),])?;
    ...
    let verify = store.perform_mut(sreq![sread!(user_ref, crate::sfield![ft_name]),])?;
...
fn test_indirection_resolution() -> Result<()> {
    ...
    // Create entities
    let create_requests = sreq![screate!(et_folder, "Security".to_string())];
    let create_requests = store.perform_mut(create_requests)?;
    let security_folder_id = if let Some(Request::Create {
        created_entity_id: Some(id),
        ..
    }) = create_requests.get(0)
    {
    ...
    let create_requests = sreq![screate!(et_user, "admin".to_string(), users_folder_ref)];
    let create_requests = store.perform_mut(create_requests)?;
    let admin_user_id = if let Some(Request::Create {
        created_entity_id: Some(id),
        ..
    }) = create_requests.get(0)
    {
    ...
    let indirect_reads = store.perform_mut(sreq![sread!(admin_user_ref, parent_name_field),])?;

    if let Some(Request::Read {
        value: Some(Value::String(name)),
        ..
    }) = indirect_reads.get(0)
    {
        assert_eq!(name.as_str(), "Users");
    } else {
...
fn test_entity_listing_with_pagination() -> Result<()> {
    ...
    for i in 0..5 {
        let create_requests = sreq![screate!(et_user, format!("user{}", i), users_folder_id)];
        store.perform_mut(create_requests)?;
    }
...
fn test_find_entities_comprehensive() -> Result<()> {
    ...
    if let Some(Request::Read {
        value: Some(Value::String(alice_name)),
        ..
    }) = name_read.get(0)
    {
    ...
    println!(
        "Name filtered results: {:?}, expected 1",
        name_filtered.len()
    );
...
fn test_find_entities_pagination() -> Result<()> {
    ...
    for i in 0..10 {
        let create_requests = sreq![screate!(et_user, format!("User{:02}", i))];
        store.perform_mut(create_requests)?;
    }
    ...
    let out_of_bounds_result =
        store.find_entities_paginated(et_user, Some(&out_of_bounds_page), None)?;
...
fn test_find_entities_inheritance() -> Result<()> {
    ...
    let mut bird_schema =
        EntitySchema::<Single, String, String>::new("Bird".to_string(), vec!["Animal".to_string()]);
    ...
    let filtered_exact_animals =
        store.find_entities_exact(et_animal, None, Some("Name == \"Rex\""))?;
...
fn test_find_entities_nonexistent_types() -> Result<()> {
    ...
    let et_nonexistent = store
        .get_entity_type("NonExistentType")
        .unwrap_or(EntityType(999999));
...
fn test_complete_entity_schema_caching() -> Result<()> {
    ...
    let mut base_schema =
        EntitySchema::<Single, String, String>::new("BaseEntity".to_string(), vec![]);
    ...
    let mut derived_schema = EntitySchema::<Single, String, String>::new(
        "DerivedEntity".to_string(),
    ...
    assert_eq!(
        complete_schema_1.fields.len(),
        complete_schema_2.fields.len()
    );
