...
#[allow(unused_imports)]
use crate::auth::{
    authenticate_user, create_user, find_user_by_name, set_user_password, AuthConfig, AuthMethod,
};
...
fn test_create_and_authenticate_user() -> Result<()> {
    ...
    // Define the schema using strings - perform_mut will intern the types
    let mut subject_schema = EntitySchema::<Single, String, String>::new(
        "Subject".to_string(),
    ...
    let user_schema = EntitySchema::<Single, String, String>::new(
        "User".to_string(),
    ...
    let parent_id = if let Some(Request::Create {
...
fn test_authentication_with_factory_restore_format() -> Result<()> {
    ...
    let create_requests =
        store.perform_mut(sreq![screate!(user_entity_type, username.to_string())])?;
    ...
    let field_requests = sreq![
        ...
        swrite!(
            user_id,
            crate::sfield![secret_field_type],
            sstr!(password_hash)
        ),
    ...
    assert_eq!(
        ...
        "Authentication should succeed and return correct user ID"
    );
