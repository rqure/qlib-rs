...
use crate::{now, sread, sreq, swrite, EntityId, Error, Request, Result, Store, Value};
...
pub fn authenticate_user(
    ...
    let user_id = find_user_by_name(store, name)?.ok_or(Error::SubjectNotFound)?;
...
pub fn authenticate_ldap(
    ...
    Err(Error::AuthenticationMethodNotImplemented(
        "LDAP".to_string(),
    ))
}
...
pub fn get_user_auth_method(store: &mut Store, user_id: EntityId) -> Result<AuthMethod> {
    let ft = match store.ft.as_ref() {
        ...
        None => {
            return Err(Error::InvalidRequest(
                "Field types not initialized".to_string(),
            ))
        }
    };
    let auth_method_ft = match ft.auth_method {
        ...
        None => {
            return Err(Error::InvalidRequest(
                "Auth method field type not initialized".to_string(),
            ))
        }
    };
...
pub fn get_user_secret(store: &mut Store, user_id: EntityId) -> Result<String> {
    ...
    let secret_ft = match ft.secret {
        ...
        None => {
            return Err(Error::InvalidRequest(
                "Secret field type not initialized".to_string(),
            ))
        }
    };
...
pub fn find_user_by_name(store: &Store, name: &str) -> Result<Option<EntityId>> {
    ...
    let user_et = match et.user {
        ...
        None => {
            return Err(Error::InvalidRequest(
                "User entity type not initialized".to_string(),
            ))
        }
    };
    ...
    let name_ft = match ft.name {
        ...
        None => {
            return Err(Error::InvalidRequest(
                "Name field type not initialized".to_string(),
            ))
        }
    };
...
pub fn is_user_active(store: &mut Store, user_id: EntityId) -> Result<bool> {
    ...
    let active_ft = match ft.active {
        ...
        None => {
            return Err(Error::InvalidRequest(
                "Active field type not initialized".to_string(),
            ))
        }
    };

    let requests = store.perform_mut(sreq![sread!(user_id, crate::sfield![active_ft])])?;
    if let Some(request) = requests.clone().read().first() {
        if let Request::Read {
...
pub fn is_user_locked(store: &mut Store, user_id: EntityId) -> Result<bool> {
    ...
    let locked_until_ft = match ft.locked_until {
        ...
        None => {
            return Err(Error::InvalidRequest(
                "Locked until field type not initialized".to_string(),
            ))
        }
    };
...
pub fn increment_failed_attempts(
    ...
    let failed_attempts_ft = match ft.failed_attempts {
        ...
        None => {
            return Err(Error::InvalidRequest(
                "Failed attempts field type not initialized".to_string(),
            ))
        }
    };
    let locked_until_ft = match ft.locked_until {
...
pub fn update_last_login(store: &mut Store, user_id: EntityId) -> Result<()> {
    ...
    let last_login_ft = match ft.last_login {
        ...
        None => {
            return Err(Error::InvalidRequest(
                "Last login field type not initialized".to_string(),
            ))
        }
    };
    let requests = sreq![swrite!(
...
pub fn create_user(
    ...
    let et = match store.et.as_ref() {
        ...
        None => {
            return Err(Error::InvalidRequest(
                "Entity types not initialized".to_string(),
            ))
        }
    };
...
pub fn authenticate_service(store: &mut Store, name: &str, secret_key: &str) -> Result<EntityId> {
    let service_id = find_subject_by_name(store, name)?.ok_or(Error::SubjectNotFound)?; // Reusing user error for services
...
pub fn find_subject_by_name(store: &mut Store, name: &str) -> Result<Option<EntityId>> {
    ...
    let entities = match et.subject {
        ...
        None => {
            return Err(Error::InvalidRequest(
                "Subject entity type not initialized".to_string(),
            ))
        }
    };
...
pub fn set_service_secret(store: &mut Store, service_id: EntityId, secret_key: &str) -> Result<()> {
    let ft = match store.ft.as_ref() {
        Some(ft) => ft,
        None => {
            return Err(Error::InvalidRequest(
...
pub fn authenticate_subject(
    ...
    match authenticate_user(store, name, credential, config) {
        ...
        Err(Error::SubjectNotFound) => match authenticate_service(store, name, credential) {
