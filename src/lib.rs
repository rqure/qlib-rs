...
pub use data::{
    build_json_entity_tree, epoch, et, extract_message_id, factory_restore_json_snapshot,
    from_base64, ft, hash_notify_config, json_value_to_value, micros_to_timestamp,
    millis_to_timestamp, nanos_to_timestamp, now, path, path_to_entity_id,
    restore_entity_recursive, restore_json_snapshot, restore_json_snapshot_via_proxy,
    secs_to_timestamp, take_json_snapshot, to_base64, value_to_json_value,
    value_to_json_value_with_paths, AdjustBehavior, ArcBlob, ArcString, AsyncStoreProxy,
    AuthenticationResult, BadIndirectionReason, Cache, Complete, EntityId, EntitySchema,
    EntityType, Field, FieldSchema, FieldType, IndirectFieldType, JsonEntity, JsonEntitySchema,
    JsonSnapshot, Notification, NotificationQueue, NotifyConfig, PageOpts, PageResult,
    PushCondition, Request, Requests, Single, Snapshot, Store, StoreMessage, StoreProxy,
    StoreTrait, Timestamp, Value, INDIRECTION_DELIMITER,
};

pub use auth::{
    authenticate_user, change_password, create_user, find_user_by_name, hash_password,
    set_user_password, validate_password, verify_password, AuthConfig, AuthMethod,
};

pub use qresp::{
    decode_message, encode_message, encode_store_message, PeerMessage, QrespCodec, QrespError,
    QrespFrame, QrespMessage, QrespMessageBuffer,
};
...
impl std::fmt::Display for Error {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Error::BadIndirection(id, field, reason) => {
                write!(f, "Bad indirection for {:?}.{:?}: {}", id, field, reason)
            }
            ...
            Error::FieldTypeNotFound(id, field) => {
                write!(f, "Field not found for {:?}: {:?}", id, field)
            }
            ...
            Error::UnsupportedAdjustBehavior(id, field, behavior) => write!(
                f,
                "Unsupported adjust behavior {:?} for {:?}.{:?}",
                behavior, id, field
            ),
            Error::InvalidRequest(msg) => write!(f, "Invalid request: {}", msg),
            Error::ValueTypeMismatch(id, field, got, expected) => write!(
                f,
                "Value type mismatch for {:?}.{:?}: got value type {:?}, expected value type {:?}",
                id, field, got, expected
            ),
            Error::BadValueCast(got, expected) => write!(
                f,
                "Bad value cast: got value type {:?}, expected value type {:?}",
                got, expected
            ),
            ...
            Error::InvalidAuthenticationMethod => {
                write!(f, "Invalid authentication method for this operation")
            }
            Error::AuthenticationMethodNotImplemented(method) => {
                write!(f, "Authentication method '{}' is not implemented", method)
            }
...
macro_rules! sstr {
    ($value:expr) => {
        Some($crate::Value::String($crate::ArcString::new(
            $value.to_string(),
        )))
    };
}
