...
use crate::{
    sread, sreq, to_base64, EntityId, IndirectFieldType, Result, Store, StoreTrait, Value,
    INDIRECTION_DELIMITER,
};
...
impl CelExecutor {
    ...
    pub fn get_or_compile(&mut self, source: &str) -> Result<&Program> {
        ...
        let program =
            Program::compile(source).map_err(|e| crate::Error::ExecutionError(e.to_string()))?;
    ...
    pub fn execute(
        ...
        for field in fields {
            ...
            let reqs = sreq![sread!(relative_id, field_types)];
            ...
            match value {
                ...
                Value::Timestamp(v) => {
                    ...
                    let datetime = chrono::DateTime::from_timestamp(unix_timestamp, nanoseconds)
                        .ok_or_else(|| {
                            crate::Error::ExecutionError("Failed to convert timestamp".to_string())
                        })?
        ...
        context.add_variable_from_value(
            "EntityType",
            store
                .resolve_entity_type(relative_id.extract_type())
                .unwrap_or_else(|_| format!("{:?}", relative_id.extract_type())),
        );
